...
const controlLinks = [
  { to: "/admin/products", icon: "🛍️", label: "إدارة المنتجات" },
  { to: "/admin/orders", icon: "📦", label: "إدارة الطلبات والحجوزات" },
  { to: "/admin/dashboard#analytics", icon: "📊", label: "الإحصائيات والتحليلات" },
];
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        showToast("تعذر تحميل الإحصائيات", "error");
      }
    };
    ...
    <div className="mx-auto max-w-6xl px-6 py-16">
      ...
      <section className="mt-10 grid gap-6 md:grid-cols-3">
        <div className="glass-card p-6">
          <p className="text-sm text-white/70">إجمالي المنتجات</p>
          <p className="mt-3 text-4xl font-black text-secondary-200">{stats.products}</p>
        </div>
        <div className="glass-card p-6">
          <p className="text-sm text-white/70">طلبات التوصيل المعلقة</p>
          <p className="mt-3 text-4xl font-black text-primary-200">{stats.pendingOrders}</p>
        </div>
        <div className="glass-card p-6">
          <p className="text-sm text-white/70">حجوزات بانتظار المتابعة</p>
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 justify-items-center md:grid-cols-3">
          {controlLinks.map((item, index) => (
            ...
            >
              ...
              <span className="mt-3 block text-lg font-semibold">{item.label}</span>
            </Link>
          ))}
        </div>
      </section>
